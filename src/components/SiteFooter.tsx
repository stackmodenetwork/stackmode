...
export const SiteFooter = memo(() => (
  <footer className="relative border-t py-12 sm:py-16 px-4" style={{
    ...
    <div className="flex flex-wrap justify-center gap-4 sm:gap-6 mb-6">
      {navLinks.map(link => link.href ? (
        <a key={link.label} href={link.href} target="_blank" rel="noopener noreferrer"
          className="text-sm tracking-wide transition-colors hover:text-neon-green"
          style={{ fontFamily: "'Orbitron', sans-serif", color: 'rgba(232,244,255,0.5)' }}>
          {link.label}
        </a>
      ) : (
    ...
    <div className="flex justify-center gap-4 mb-8">
      {socials.map(s => (
        <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.label}
          className="text-lg sm:text-xl opacity-50 hover:opacity-100 transition-opacity hover:scale-110 transform">
          {s.icon}
        </a>
      ))}
    </div>
    ...
    <div className="flex justify-center mb-8">
      <div className="trustpilot-widget" data-locale="en-US" data-template-id="56278e9abfbbba0bdcd568bc"
        ...
        <a href="https://www.trustpilot.com/review/stackmode.net" target="_blank" rel="noopener noreferrer"
          className="text-xs" style={{ color: 'rgba(232,244,255,0.3)' }}>Trustpilot</a>
      </div>
    </div>

    {/* Book links */}
    <div className="flex flex-wrap justify-center gap-3 mb-8">
      <a href="https://stackmode.net" className="btn-ghost text-[10px] py-2 px-4">
        📚 Amazon Books →
      </a>
      <a href="https://stackmode.net" className="btn-ghost text-[10px] py-2 px-4">
        📖 Google Play Books →
    ...
    {/* Legal */}
    <div className="flex flex-wrap justify-center gap-4 text-xs mb-4" style={{ color: 'rgba(232,244,255,0.3)' }}>
    ...
    {/* SEO content strip */}
    <p className="text-center text-[10px] mt-4 max-w-2xl mx-auto" style={{
    ...
    <p className="text-center text-[9px] mt-2" style={{ color: 'rgba(232,244,255,0.15)' }}>
