...
export const TradingAuditCTA = () => {
  ...
  return (
    <section className="relative z-10 px-4 py-10 sm:py-14">
      <motion.div
      ...
      >
        ...
        <div className="text-center mb-6">
          <motion.div
            className="inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/40 rounded-full px-4 py-2"
            initial={{ opacity: 0, scale: 0.9 }}
            whileInView={{ opacity: 1, scale: 1 }}
            viewport={{ once: true }}
          >
            <Calendar size={16} className="text-emerald-400" />
            <span className="text-emerald-400 text-sm font-bold tracking-wide">FREE 1-ON-1 TRADING AUDIT</span>
          </motion.div>
        </div>

        {/* Main Headline */}
        <motion.h2
          className="text-2xl sm:text-3xl md:text-4xl font-bold text-foreground text-center mb-4 leading-tight"
        ...
        {/* Subheadline */}
        <motion.p
          className="text-base sm:text-lg text-muted-foreground text-center max-w-2xl mx-auto mb-8"
        ...
        {/* Main Card */}
        <div className="relative bg-gradient-to-br from-emerald-500/10 via-card/95 to-primary/10 border-2 border-emerald-500/40 rounded-2xl sm:rounded-3xl p-6 sm:p-8 overflow-hidden">
          {/* Background glow */}
            ...
            <div className="mb-6">
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                {auditPoints.map((point, i) => (
                  <motion.div
                    key={i}
                    className="flex items-start gap-3 bg-background/50 border border-emerald-500/20 rounded-xl p-4"
                    initial={{ opacity: 0, y: 15 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-6 border-t border-emerald-500/20">
              ...
              <div className="flex items-center gap-3 text-center sm:text-left">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Only 5 calls per week — serious traders only</p>
                </div>
              </div>
