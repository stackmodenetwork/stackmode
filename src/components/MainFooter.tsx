import { Link } from 'react-router-dom';
export const MainFooter = () => {
  return (
    <footer className="bg-background border-t border-border/50 py-8 px-4 pb-bottom-nav lg:pb-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-wrap justify-center gap-4 text-sm mb-6">
          <Link to="/" className="text-muted-foreground hover:text-foreground transition-colors">Home</Link>
          <span className="text-border/50 hidden sm:inline">•</span>
          <Link to="/coding" className="text-muted-foreground hover:text-foreground transition-colors">Academy</Link>
          <span className="text-border/50 hidden sm:inline">•</span>
          <Link to="/buildyourwebsite" className="text-muted-foreground hover:text-foreground transition-colors">Build With Us</Link>
          <span className="text-border/50 hidden sm:inline">•</span>
          <Link to="/library" className="text-muted-foreground hover:text-foreground transition-colors">Library</Link>
          <span className="text-border/50 hidden sm:inline">•</span>
          <Link to="/about" className="text-muted-foreground hover:text-foreground transition-colors">About</Link>
        </div>
        
        <div className="flex flex-wrap justify-center gap-4 text-xs text-muted-foreground/70 mb-6">
          <Link to="/privacy" className="hover:text-muted-foreground transition-colors">Privacy</Link>
          <span className="text-border/30">•</span>
          <Link to="/terms" className="hover:text-muted-foreground transition-colors">Terms</Link>
          <span className="text-border/30">•</span>
          <Link to="/dmca" className="hover:text-muted-foreground transition-colors">DMCA</Link>
        </div>

        <div className="flex justify-center gap-4 mb-6">
          <a href="https://discord.gg/5zYWSWGMYm" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-cyan-400 transition-colors" aria-label="Discord">
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03z" /></svg>
          </a>
          <a href="https://www.youtube.com/@Stackmodechris" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-red-400 transition-colors" aria-label="YouTube">
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" /></svg>
          </a>
        </div>

        <div className="border-t border-border/30 pt-4 mb-4">
          <p className="text-xs text-muted-foreground text-center mb-2">
            Need to grow your brand beyond code? <a href="https://ceoturbo.com" target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:text-cyan-300 font-medium transition-colors">CEOTurbo.com</a> — brand strategy, marketing systems & business growth tools.
          </p>
        </div>

        <p className="text-xs text-muted-foreground/50 text-center">
          Stackmode Academy | Founded by Christopher Robinson | StackmodeChris | The #1 Academy for AI Software, Trading & Asset Stacking | stackmode.net
        </p>
        <p className="text-[10px] text-muted-foreground/30 text-center mt-1">© {new Date().getFullYear()} Stackmode Academy. All rights reserved.</p>
      </div>
    </footer>
  );
};
export default MainFooter;
