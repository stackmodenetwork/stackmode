...
const WireframeCanvas = () => {
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      if (layoutIdx === 0) {
        // Landing page: nav + hero + CTA
        rc(vpX, vpY, vpW, 8); // nav
        for (let i = 0; i < 4; i++) rc(vpX + vpW - 4 * 14 + i * 14, vpY + 1, 10, 5); // nav items
        rc(vpX, vpY + 14, vpW, vpH * 0.5); // hero
        ctx.fillStyle = 'rgba(0,207,255,0.1)';
        rc(vpX + vpW / 2 - 25, vpY + 14 + vpH * 0.55, 50, 10, 3); // CTA
      } else if (layoutIdx === 1) {
        // Card grid 3x2
        ...
        // Bar chart
        ctx.fillStyle = 'rgba(0,207,255,0.08)';
        const barH = [0.6, 0.8, 0.5, 0.9, 0.7];
...
const LeftPanel = ({ hovered, otherHovered, onHover }: Omit<PanelProps, 'side'>) => {
  ...
  return (
    ...
    >
      ...
      {/* Corner brackets */}
      <div className="absolute top-5 left-5 w-5 h-5 border-l-[1.5px] border-t-[1.5px]" style={{ borderColor: 'rgba(0,255,136,0.25)' }} />
      <div className="absolute bottom-5 right-5 w-5 h-5 border-r-[1.5px] border-b-[1.5px]" style={{ borderColor: 'rgba(0,255,136,0.25)' }} />

      {/* Content */}
      <div className="relative z-10 flex flex-col items-start px-8 sm:px-12 max-w-lg" style={{ animation: 'fadeUp 0.8s ease 0.2s both' }}>
        {/* Tag */}
        <span className="mb-4 px-3 py-1 text-[9px] tracking-[0.2em] uppercase rounded-full" style={{
          fontFamily: "'Share Tech Mono', monospace",
          color: '#00ff88',
        ...
        <h2 style={{
          ...
          STACK<br /><span style={{ color: '#00ff88' }}>MODE</span><br />ACADEMY
        </h2>

        {/* Description — fades on hover (desktop) or always visible (mobile) */}
        <p className={`mt-4 transition-all duration-500 md:${hovered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}
          style={{
        ...
        {/* Pills */}
        <div className={`flex flex-wrap gap-2 mt-3 transition-all duration-500 ${hovered ? 'opacity-100' : 'md:opacity-0'}`}>
          {['Python', 'Trading', 'Web Dev', 'AI Tools', 'Monetize'].map(p => (
            <span key={p} className="text-[8px] tracking-wider px-2 py-0.5" style={{
              fontFamily: "'Share Tech Mono', monospace",
        ...
        {/* CTA */}
        <div className="relative mt-6 overflow-hidden group/cta" style={{
          ...
          <span className="relative z-10 text-[11px] tracking-[0.25em] transition-colors duration-300 group-hover/cta:text-black" style={{
            fontFamily: "'Share Tech Mono', monospace",
            color: '#00ff88',
          }}>
            JOIN ACADEMY →
...
const RightPanel = ({ hovered, otherHovered, onHover }: Omit<PanelProps, 'side'>) => {
  ...
  return (
    ...
    >
      ...
      {/* Corner brackets */}
      <div className="absolute top-5 right-5 w-5 h-5 border-r-[1.5px] border-t-[1.5px]" style={{ borderColor: 'rgba(0,207,255,0.25)' }} />
      <div className="absolute bottom-5 left-5 w-5 h-5 border-l-[1.5px] border-b-[1.5px]" style={{ borderColor: 'rgba(0,207,255,0.25)' }} />

      {/* Content */}
      <div className="relative z-10 flex flex-col items-start px-8 sm:px-12 max-w-lg" style={{ animation: 'fadeUp 0.8s ease 0.4s both' }}>
        <span className="mb-4 px-3 py-1 text-[9px] tracking-[0.2em] uppercase rounded-full" style={{
          fontFamily: "'Share Tech Mono', monospace",
          color: '#00cfff',
        ...
        <h2 style={{
          ...
          CEO<br /><span style={{ color: '#00cfff' }}>TURBO</span><br />.COM
        </h2>

        <p className={`mt-4 transition-all duration-500`}
          style={{
            fontFamily: "'Syne', sans-serif",
            fontWeight: 700,
            fontSize: 12,
