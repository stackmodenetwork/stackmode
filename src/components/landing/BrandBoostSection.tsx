      ...
      <div ref={ref} className="relative z-10 max-w-6xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-20">
          <div className={`flex-1 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <h2 className="text-3xl sm:text-4xl md:text-5xl leading-[1.1] mb-6" style={{ fontFamily: "'Bebas Neue', sans-serif", color: '#FF6B2B' }}>
              YOUR BRAND IS MAKING<br />AN IMPRESSION RIGHT NOW.
              <br /><span style={{ color: '#A0A0B8' }}>IS IT THE RIGHT ONE?</span>
            </h2>
          ...
          <div className={`flex-1 transition-all duration-700 delay-200 ${visible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12'}`}>
            <div className="rounded-lg border p-6 sm:p-8" style={{ borderColor: 'rgba(255,107,43,0.15)', background: 'rgba(255,107,43,0.02)' }}>
              ...
              <div className="space-y-4">
                {checklist.map((item, i) => (
                  <div key={item} className="flex items-center gap-3 transition-all duration-300" style={{ opacity: i < checkedCount ? 1 : 0.2 }}>
                    <span className="w-5 h-5 rounded flex items-center justify-center text-xs border transition-all duration-300"
                      style={{
                        borderColor: i < checkedCount ? '#FF6B2B' : 'rgba(255,255,255,0.1)',
                        color: i < checkedCount ? '#FF6B2B' : 'transparent',
                        background: i < checkedCount ? 'rgba(255,107,43,0.1)' : 'transparent',
                      }}>✓</span>
