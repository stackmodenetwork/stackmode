  ...
  return (
    <section className="py-16 sm:py-20 overflow-hidden">
      ...
      <div className="relative flex h-[600px] w-full items-start justify-center overflow-hidden gap-3 px-4 max-w-6xl mx-auto">
        {/* Column 1 - down */}
        <Marquee vertical pauseOnHover repeat={1} className="[--duration:90s] [--gap:0.75rem] flex-1">
          {col1.map((img) => (
            <ReviewImg key={img} src={img} onClick={() => openImage(img)} />
          ))}
        </Marquee>
        {/* Column 2 - up */}
        <Marquee vertical reverse pauseOnHover repeat={1} className="[--duration:80s] [--gap:0.75rem] flex-1">
          {col2.map((img) => (
            <ReviewImg key={img} src={img} onClick={() => openImage(img)} />
          ))}
        </Marquee>
        {/* Column 3 - down */}
        <Marquee vertical pauseOnHover repeat={1} className="[--duration:100s] [--gap:0.75rem] flex-1 hidden sm:flex">
          {col3.map((img) => (
            <ReviewImg key={img} src={img} onClick={() => openImage(img)} />
          ))}
        </Marquee>
        {/* Column 4 - up */}
        <Marquee vertical reverse pauseOnHover repeat={1} className="[--duration:85s] [--gap:0.75rem] flex-1 hidden lg:flex">
          {col4.map((img) => (
            <ReviewImg key={img} src={img} onClick={() => openImage(img)} />
          ))}
        </Marquee>
